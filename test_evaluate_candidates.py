...
def evaluate_set(prefix, tails, annoy_tree, vector_dims, rank_threshold=100, sample_size=1000):
    #prefix = "blabla"
    #tails = set([(8,9), (10,11), (12,13)])
    print time.time(), "Start ", mp.current_process().pid
    counts = dict()
    counts[True] = 0
    counts[False] = 0
    ...
    print time.time(), "End ", mp.current_process().pid
    time.sleep(3)
...
def candidate_generator(candidates, annoy_tree, vector_dims, rank_threshold, sample_size):
    for prefix in candidates:
        yield (prefix, candidates[prefix], annoy_tree, vector_dims, rank_threshold, sample_size)
...
def evaluate_candidates(candidates, annoy_tree_file, vector_dims, rank_threshold=100, sample_size=500, processes=4):
    ...
    arguments = candidate_generator(candidates, annoy_tree, vector_dims, rank_threshold, sample_size)
